...
function analitzarTemperatures(...temperaturas) {
    const tabla = document.getElementById('taulaTemperatures');
    ...
    const sumTotal = temperaturas.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
    const tempMitjana = parseFloat((sumTotal / total).toFixed(2));
    celda1.textContent = String(tempMitjana.toFixed(2));
    celda2.textContent = String(tempMax);
    celda3.textContent = String(tempMin);
}
function enviarMissatges(input1, input2) {
